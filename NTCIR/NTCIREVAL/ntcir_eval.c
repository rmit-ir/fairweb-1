...
static double goldprob[ NGROUPS_MAX ]; /* gold probabilites for GFR */
...
static int eval_GFlabel( int, char** );
static int eval_GFRcompute( int, char** );
...
static struct {
  ...
  { "GFlabel", eval_GFlabel },
  { "GFRcompute", eval_GFRcompute },
  { "1click", eval_1click },
  {(char *) 0, (int (*)()) 0}
};
...
int main( int ac, char **av )
{
  ...
  if( ac < 2 ){
    ...
    fprintf(stderr, " command= glabel|dinlabel|gcompute|irec|label|compute|GFlabel|GFRcompute|1click\n\n" );
    ...
    fprintf(stderr, "    outputs evaluation metric values.\n\n" );
    fprintf(stderr, "   *GFlabel* reads a list of docs with group membership\n" );
    fprintf(stderr, "    probabilities and a system ranked list;\n" );
    fprintf(stderr, "    outputs a system ranked list WITH probabilities.\n\n" );
    fprintf(stderr, "   *GFRcompute* reads a list of judged docs with rel levels\n" );
    fprintf(stderr, "    and a system ranked list WITH grop probabilities;\n" );
    fprintf(stderr, "    outputs GFR measure values.\n\n" );
...
reads a group membership file and a ranked list of documents;
outputs a labelled ranked list - each doc with group membership probabilities.
If a doc is missing from the group membership file,
*that doc is given a uniform distribution* in the output.

group membership file format:
<docID> <prob>...<prob>
<docID> <prob>...<prob>
  :
(May contain empty lines or comments beginning with #. They will be ignored.)  
...
return value: 0 (OK)
             -1 (NG)
******************************************************************************/
static int eval_GFlabel( int ac, char **av )
{
  FILE *fa = NULL;
  FILE *fs = stdin;
  ...
  /* by default, do not truncate */
  long i, j;
  long ngroups = 0; /* number of groups for group membership (REQUIRED) */
  /* used for obtaining a uniform distribution for each nonrelevant doc */

  char line[ BUFSIZ + 1 ];
  char *pos;

  struct strstrlonglist *jdocgprobs = NULL; /* judged doc with group probs */
  struct strstrlonglist *p;

  long file2strstrcountlist2();
  int freestrstrlonglist();

  while( argc < ac ){
    if( strcmp( av[ argc ], OPTSTR_TRUNCATE ) == 0 && ( argc + 1 < ac ) ){
      truncaterank = atol( av[ argc + 1 ] );
      if( truncaterank < 1 ){
    ...
    /* number of groups */
    else if( strcmp( av[ argc ], OPTSTR_NGROUPS ) == 0 && ( argc + 1 < ac ) ){
      ngroups = atol( av[ argc + 1 ] );
      if( ngroups < 2 || ngroups > NGROUPS_MAX ){
    ...
    else if( strcmp( av[ argc ], OPTSTR_GMFILE ) == 0 && ( argc + 1 < ac ) ){
      if( ( fa = fopen( av[ argc + 1 ], "r" ) ) == NULL ){
  ...
  if( ngroups == 0 || fa == NULL ){

#ifdef OUTERR      
    fprintf( stderr,
	     "Usage: %s %s [%s <separator>] [%s <rank>] %s <group_membership_file> %s <ngroups> [ranked list file]\n",
	     av[ 0 ], av[ 1 ],
	     OPTSTR_SEP, OPTSTR_TRUNCATE, 
	     OPTSTR_GMFILE, OPTSTR_NGROUPS );
#endif
    return( 0 );
  }
  ...
  if( file2strstrcountlist2( fa, &jdocgprobs ) < 0 ){
#ifdef OUTERR
    fprintf( stderr, "file2strstrcountlist2 failed\n" );
#endif
    return( -1 );
  }
  ...
  while( fgets( line, sizeof( line ), fs ) ){
    ...
    *pos = '\0';
	...
	printf( "%s%s%s\n", line, sep, p->string2 ); /* with group probs */
	match = 1;
    ...
    if( match == -1 ){ /* doc not found in group membership file */

      /* output a uniform distribution */
      printf( "%s", line );
      for( j = 1; j <= ngroups; j++ ){
	printf( "%s%f", sep, 1/((double)ngroups) );
      }
      printf( "\n" );
...
reads a relevance assessment file and a ranked list with groupmemberships;
outputs Group Fairness and Relevance scores.
...
ranked list with groupmemberships format:
<docID> <prob1> ... <prob|C|>
<docID> <prob1> ... <prob|C|>
<docID> <prob1> ... <prob|C|>
 :
...
******************************************************************************/
static int eval_GFRcompute( int ac, char **av )
{
  ...
  long ngroups = 0; /* GFR: specified number of groups */
  long nprobs;  /* GFR: #lines in gold distr file (should equal ngroups) */

  double gv[ MAXRL_MAX + 1 ]; /* gain value for an X-relevant doc */
  double *Gs; /* system gain value */
  ...
  double *GFJSD; /* at each rank */
  double *GFNMD; /* at each rank */
  double *GFRNOD; /* at each rank */

  char *buf, *p;

  /*  struct strstrlist *sysdoclab = NULL; */
  struct strdoub3list *sysdocdiv3 = NULL; /* doc, JSD, NMD, RNOD */
  struct strdoub3list *docdiv3;
  struct strlonglist *reldoc_rlevel = NULL;

  int GFRcompute_usage();
  int freestrlonglist();
  int freestrdoub3list();
  int fprintstrdoub3list();
  int fprintstrlonglist();

  /*  long count_judged(); */
  long count_ECjudged();
  long doc2rlevel_fromlist();
  long store_doc_divergences();
  long store_golddistr();
  long store_reldoc_rlevel();
  double orig_dcglog();

  while( argc < ac ){

    if( strcmp( av[ argc ], OPTSTR_HELP ) == 0 ){
      return( GFRcompute_usage( ac, av ) );
    }
    else if( strcmp( av[ argc ], OPTSTR_VERBOSE ) == 0 ){
      verbose = 1;
      argc++;
    ...
    /* gold distribution file */
    else if( strcmp( av[ argc ], OPTSTR_GDISTR ) == 0 && ( argc + 1 < ac ) ){
      if( ( fg = fopen( av[ argc + 1 ], "r" ) ) == NULL ){
    ...
    /* system file with group probabilities */
    else{
      if( ( fs = fopen( av[ argc ], "r" ) ) == NULL ){
  ...
  if( fa == NULL || fg == NULL|| maxrl == 0 || ngroups == 0 ){ 
/* rel file, gold distr file, gain values, and #groups are required */
    return( GFRcompute_usage( ac, av ) );
  }

  /* store the gold array */
  if( ( nprobs = store_golddistr( fg ) ) != ngroups ){
    /* number of lines in gold file does not match ngroups from command line*/

#ifdef OUTERR
    fprintf( stderr, "ngroups=%ld but nprobs=%ld\n", ngroups, nprobs );
#endif
    return( -1 );
  ...
  /* check that gain values for lower relevance levels are lower */
  for( i = 1; i <= maxrl; i++ ){
    for( j = i + 1; j <= maxrl; j++ ){
      if( gv[ i ] > gv[ j ] ){

#ifdef OUTERR
	fprintf( stderr, "Bad gain values\n" );
#endif
	return( -1 );
  ...
  if( ( jrelnum = store_reldoc_rlevel( maxrl, Xrelnum, fa, 
				       &reldoc_rlevel ) ) < 0 ){

#ifdef OUTERR
    fprintf( stderr, "store_reldoc_rlevel failed\n" );
#endif
    return( -1 );
  }
  fclose( fa );
  ...
  if( verbose ){
    printf( "#store_reldoc_rlevel result:\n" );
    fprintstrlonglist( stdout, reldoc_rlevel );
  }
  ...
  /* obtain divergence scores of each doc, use gold distr 
    (store docIDs with (JSD, NMD, RNOD) as strdoub3list) */

  if( ( syslen = 
	store_doc_divergences( fs, ngroups, &sysdocdiv3 ) ) < 0 ){

#ifdef OUTERR
    fprintf( stderr, "store_doc_divergences failed\n" );
#endif
    return( -1 );
  }
  ...
  if( verbose ){
    printf( "#store_doc_divergences result:\n" );
    fprintstrdoub3list( stdout, sysdocdiv3 );
  }
  ...
  GFJSD = ( double *)calloc( maxlen + 1, sizeof( double ) );
  GFNMD = ( double *)calloc( maxlen + 1, sizeof( double ) );
  GFRNOD = ( double *)calloc( maxlen + 1, sizeof( double ) );
  ...
  ERR[ 0 ] = 0;
  iRBU[ 0 ] = 0;
  GFJSD[ 0 ] = 0; GFNMD[ 0 ] = 0; GFRNOD[ 0 ] = 0;
  ...
  for( i = 1; i<= syslen; i++ ){
    ...
    if( ( rlevel = doc2rlevel_fromlist( reldoc_rlevel, 
					docdiv3->string ) ) >=0 ){
      /* judged */
    ...
    /* if rlevel < 0 unjudged => do nothing */
    ...
    */
    ERR[ i ] = ERR[ i - 1 ] + 
      ( (double)1/i ) * ERRdsat[ i - 1 ]*Gs[ i ]/( gv[ maxrl ]+1 );
    /*      ( ERRdsat[ i - 1 ]/i ) * ( Gs[ i ]/( gv[ maxrl ]+1 ) ); */
    iRBU[ i ] = iRBU[ i - 1 ] + 
      pow( RBUp, i ) * ERRdsat[ i - 1 ]*Gs[ i ]/( gv[ maxrl ]+1 );

    /* note that val1 etc are divergences so we need to convert them
       into similarities: sim=1-divergence */
    GFJSD[ i ] = GFJSD[ i - 1 ] +
      ( 1 - docdiv3->val1 ) * ERRdsat[ i - 1 ]*Gs[ i ]/( gv[ maxrl ]+1 );
    GFNMD[ i ] = GFNMD[ i - 1 ] +
      ( 1 - docdiv3->val2 ) * ERRdsat[ i - 1 ]*Gs[ i ]/( gv[ maxrl ]+1 );
    GFRNOD[ i ] = GFRNOD[ i - 1 ] +
      ( 1 - docdiv3->val3 ) * ERRdsat[ i - 1 ]*Gs[ i ]/( gv[ maxrl ]+1 );
    

    ERRdsat[ i ] = ERRdsat[ i - 1 ] * ( 1 - Gs[ i ]/( gv[ maxrl ]+1 ) );
    docdiv3 = docdiv3->next;
  }

  freestrdoub3list( &sysdocdiv3 );
  freestrlonglist( &reldoc_rlevel );

  for( i = syslen + 1; i <= maxlen; i++ ){ /* if system size < ideal size */

    ERR[ i ] = ERR[ syslen ];
    iRBU[ i ] = iRBU[ syslen ];
    GFJSD[ i ] = GFJSD[ syslen ];
    GFNMD[ i ] = GFNMD[ syslen ];
    GFRNOD[ i ] = GFRNOD[ syslen ];
  ...
  for( i = 0; i < cutoff_num; i++ ){
    ...
    if( cutoff[ i ] <= maxlen ){
      ...
      printf( "%s iRBU@%04ld=           %.4f\n",
	      outstr, cutoff[ i ], iRBU[ cutoff[ i ] ] );
      printf( "%s GF-JSD@%04ld=         %.4f\n",
	      outstr, cutoff[ i ], GFJSD[ cutoff[ i ] ] );
      printf( "%s GF-NMD@%04ld=         %.4f\n",
	      outstr, cutoff[ i ], GFNMD[ cutoff[ i ] ] );
      printf( "%s GF-RNOD@%04ld=        %.4f\n",
	      outstr, cutoff[ i ], GFRNOD[ cutoff[ i ] ] );
      ...
      printf( "%s iRBU@%04ld=           %.4f\n",
	      outstr, cutoff[ i ], iRBU[ maxlen ] );
      printf( "%s GF-JSD@%04ld=         %.4f\n",
	      outstr, cutoff[ i ], GFJSD[ maxlen ] );
      printf( "%s GF-NMD@%04ld=         %.4f\n",
	      outstr, cutoff[ i ], GFNMD[ maxlen ] );
      printf( "%s GF-RNOD@%04ld=        %.4f\n",
	      outstr, cutoff[ i ], GFRNOD[ maxlen ] );
    }
  }
...
lookup the rlevel of a given doc.
if doc is not on the list that is fine. Just return -1.

return value: rlevel (if doc is found)
              -1 (not found)
 *****************************************************************************/
long doc2rlevel_fromlist( list, doc )
     struct strlonglist *list; /* doc+rlevel */
     char* doc;
{
  long retv = -1;
  struct strlonglist *p;

  p = list;
  while( p ){
    if( !strcmp( p->string, doc ) ){
      return( p->val );
    }
    p = p->next;
  }
...
/******************************************************************************
 store_doc_divergences

read a doc list with group membership probabilities

input file format:
<doc> <prob1> ... <prob|C|>
<doc> <prob1> ... <prob|C|>
where |C| is the number of groups given in the argument.

and for each doc, compute an achieved distribution
and compute divergences: JSD, NMD, RNOD.

If a line contains more than |C| values, the redundant values are ignored.

Store them in a strddoublist.
<doc> <val2(JSD)> <val2(NMD)> <val3(RNOD)>
 :

Return value: number of docs read

 *****************************************************************************/
long store_doc_divergences( f, ng, list )
     FILE *f;
     long ng; /* number of groups for checking each line (|C|)*/
     struct strdoub3list **list;
{
  long retv = 0;
  long i, groupcount;
  double jsd, nmd, rnod;
  ...
  double cumprob[ NGROUPS_MAX ]; /* sum of probabilities for each group */
  double aveprob[ NGROUPS_MAX ]; /* average of the above across ranks */

  struct strdoub3list **pp1, **pp2;

  struct strdoub3list **addtostrdoub3list();

  double sysgold2jsd();
  double sysgold2nmd();
  double sysgold2rnod();

  if( ng < 2 || ng > NGROUPS_MAX ){
#ifdef OUTERR
        fprintf( stderr, "Bad ng value: %ld\n", ng );
#endif
        return( -1 );
  }
  ...
  while( fgets( line, sizeof( line ), f ) ){
    ...
    *pos = '\0';
    ...
    groupcount = 0;
    for( i = 1; i <= ng; i++ ){ /* ignore if there are more than ng values */
      ...
      cumprob[ groupcount ] += atof( p );
      aveprob[ groupcount ] = cumprob[ groupcount ]/(double)(retv+1);
      
      groupcount++;

    }

    if( ( jsd = sysgold2jsd( ng, aveprob, goldprob ) ) < 0.0 ){
#ifdef OUTERR   
      fprintf( stderr, "sysgold2jsd failed\n" );
#endif  
      return( -1 );
    }

    if( ( nmd = sysgold2nmd( ng, aveprob, goldprob ) ) < 0.0 ){
#ifdef OUTERR   
      fprintf( stderr, "sysgold2nmd failed\n" );
#endif  
      return( -1 );
    }

    if( ( rnod = sysgold2rnod( ng, aveprob, goldprob ) ) < 0.0 ){
#ifdef OUTERR   
      fprintf( stderr, "sysgold2rnod failed\n" );
#endif  
      return( -1 );
    }

    if( ( pp2 =
	  addtostrdoub3list( pp1, buf, jsd, nmd, rnod ) ) == NULL ){

#ifdef OUTERR   
      fprintf( stderr, "addtostrdoub3list failed for %s\n", buf );
#endif  
      return( -1 );
...
}/* of store_doc_divergences */

/******************************************************************************
sysgold2jsd

 compute JSD from system and gold distribution with ng classes

return value: jsd (-1.0 if error)
 *****************************************************************************/
double sysgold2jsd( ng, sysprob, goldprob )
     long ng; /* number of groups */
     double sysprob[];
     double goldprob[];
{
  double retv;
  double kld1, kld2;
  long i;
  double pivot[ NGROUPS_MAX ];

  double kldivergence();

  if( ng < 2 ) return( -1 );

  /* compute pivot distribution */
  for( i = 0; i < ng; i++ ){
    pivot[ i ] = ( sysprob[ i ] + goldprob[ i ] )/2;
  }

  kld1 = kldivergence( ng, sysprob, pivot );
  kld2 = kldivergence( ng, goldprob, pivot );
...
return value: kld (-1.0 if error)
 *****************************************************************************/
double kldivergence( ng, prob1, prob2 )
     long ng; /* number of groups */
     double prob1[];
     double prob2[];
{
  ...
  for( i = 0; i < ng; i++ ){
    if( prob1[ i ] > 0 ){
      ...
      if( prob2[ i ] == 0.0 ) return( -1 ); /* this will not happen for JSD */

      retv += prob1[ i ] * log( prob1[ i ]/prob2[ i ] )/log( 2 );
...
sysgold2nmd
 ...
 compute nmd from system and gold distribution with ng classes

return value: nmd (-1.0 if error)
 *****************************************************************************/
double sysgold2nmd( ng, sysprob, goldprob )
     long ng; /* number of groups */
     double sysprob[];
     double goldprob[];
{
  ...
  syscum[ 0 ] = sysprob[ 0 ];
  goldcum[ 0 ] = goldprob[ 0 ];

  for( i = 1; i < ng; i++ ){

    syscum[ i ] = syscum[ i - 1 ] + sysprob[ i ];
    goldcum[ i ] = goldcum[ i - 1 ] + goldprob[ i ];

  }

  for( i = 0; i < ng; i++ ){

    if( syscum[ i ] >= goldcum[ i ] ){
      sum += syscum[ i ] - goldcum[ i ]; 
    }
    else{
      sum += goldcum[ i ] - syscum[ i ];
    }
...
sysgold2rnod
 ...
 compute rnod from system and gold distribution with ng classes

return value: rnod (-1.0 if error)
 *****************************************************************************/
double sysgold2rnod( ng, sysprob, goldprob )
     long ng; /* number of groups */
     double sysprob[];
     double goldprob[];
{
  ...
  double od = 0.0;
  long i, j;
  long npositive = 0; /* #positive groups in gold distribution: |C*| */
  double dw[ ng ];

  /* compute distance weighted sum of squares */
  for( i = 0; i < ng; i++ ){

    dw[ i ] = 0;
    for( j = 0; j < ng; j++ ){
      if( i == j ){
	/* omit the calculation */
      }
      else if( i > j ){
	dw[ i ] += 
	  ( i - j ) * ( sysprob[ j ] - goldprob[ j ] ) * ( sysprob[ j ] - goldprob[ j ] );
      }
      else{ /* i < j */
	dw[ i ] += 
	  ( j - i ) * ( sysprob[ j ] - goldprob[ j ] ) * ( sysprob[ j ] - goldprob[ j ] );
      }
    }
...
read a gold distribution file (one gold probability per line) and store it in the static array.

Do not care about how the probabilities add up.

Return value: number of probabilities read.
              0 (ERROR)

 *****************************************************************************/
long store_golddistr( f )
     FILE *f; /* i */
{
  long retv = 0;
  char line[ BUFSIZ + 1 ];
...
/******************************************************************************
store_reldoc_rlevel (extension of count_judged)
...
AND store the docID and the relevance level (long) in a strlonglist

return value: number of judged rel docs >=0 (OK)
             -1 (ERROR)
******************************************************************************/
long store_reldoc_rlevel( maxrl, Xrel, f, list )
     long maxrl;
     long Xrel[];/* number of X-rel docs */
     FILE *f; /* rel assessment file */
     struct strlonglist **list; /* store doc and level */
{
  long i;
  long jrel; /* return value */
  ...
  struct strlonglist **pp1, **pp2;

  long lab2level();
  struct strlonglist **addtostrlonglist();
  ...
  while( fgets( line, sizeof( line ), f ) ){
    ...
    *pos = '\0';
    ...
    if( ( pp2 = addtostrlonglist( pp1, buf, i ) ) == NULL ){

#ifdef OUTERR   
      fprintf( stderr, "addtostrlonglist failed for %s\n", buf );
#endif  
      return( -1 );
    }
    pp1 = pp2;
     ...
     char **av;
{
...
#ifdef OUTERR  
  fprintf( stderr,
	   "Usage: %s %s %s <relfile> %s <gainL1:gainL2...> %s <ngroups> %s <goldfile> [OPTIONS] [labelled ranked list]\n",
	   av[ 0 ], av[ 1 ], OPTSTR_RELFILE, OPTSTR_GAIN,
	   OPTSTR_NGROUPS, OPTSTR_GDISTR );
  fprintf( stderr, "OPTIONS:\n" );
  fprintf( stderr, "\t%s: show this message\n", OPTSTR_HELP );
  fprintf( stderr, "\t%s: verbose output\n", OPTSTR_VERBOSE );
  fprintf( stderr, "\t%s <separator>: input/output field separator (default: '%s').\n",
  ...
  fprintf( stderr, "\t%s <string>: prefix string for each output line.\n",
	   OPTSTR_OUTSTR );
  fprintf( stderr, "\t    Including a topicID here would be useful.\n" );
  fprintf( stderr, "\t%s <positive value <=1 >: persistence for iRBU (default: %.2f)\n",
	   OPTSTR_RBUP, DEFAULT_RBU );
  fprintf( stderr,
	   "\t%s <document rank[,document rank,...]>: cutoffs for ERR@n... (default: %d)\n",
	   OPTSTR_CUTOFF, DEFAULT_CUTOFF );
#endif
...
return value: 0 (OK)
******************************************************************************/
int freestrlonglist( p )
     struct strlonglist **p;
{
  struct strlonglist *p1, *p2;
...
add <str,long> to end of list

return value: <lastrecord>->next
              NULL (error)
******************************************************************************/
struct strlonglist **addtostrlonglist( p, s, val )
     struct strlonglist **p;
     char *s; /* must be allocated outside */
     long val;
{
  struct strlonglist *q;

  if( s == NULL )
    return( NULL );

  if( ( q = ( struct strlonglist * )
	malloc( sizeof( struct strlonglist ) ) ) == NULL ){
...
/******************************************************************************
 addtostrdoublonglist

add <str,double, long> to end of list

return value: <lastrecord>->next
              NULL (error)
******************************************************************************/
struct strdoublonglist **addtostrdoublonglist( p, s, val1, val2 )
     struct strdoublonglist **p;
     char *s; /* must be allocated outside */
     double val1;
     long val2;
...
return value: 0 (OK)
******************************************************************************/
int freestrdoub3list( p )
     struct strdoub3list **p;
{
  struct strdoub3list *p1, *p2;
...
/******************************************************************************
  fprintstrdoublong2list
return value: 0 (OK)  
******************************************************************************/
int fprintstrdoublong2list( fp, p )
     FILE *fp;
     struct strdoublong2list *p;
{
  struct strdoublong2list *p1;

  p1 = p ;

  while( p1 ){
    fprintf( fp, "%s %.4f %ld %ld\n", 
	     p1->string, p1->val1, p1->val2, p1->val3 );
    p1 = p1->next;
  }
  
  return( 0 );

}/* of fprintstrdoublong2list */

/******************************************************************************
  fprintstrdoub3list
return value: 0 (OK)  
******************************************************************************/
int fprintstrdoub3list( fp, p )
     FILE *fp;
     struct strdoub3list *p;
{
  struct strdoub3list *p1;
...
}/* of fprintstrdoub3list */

/******************************************************************************
  fprintstrdoublonglist
return value: 0 (OK)  
******************************************************************************/
int fprintstrdoublonglist( fp, p )
     FILE *fp;
     struct strdoublonglist *p;
{
  struct strdoublonglist *p1;

  p1 = p ;

  while( p1 ){
    fprintf( fp, "%s %.4f %ld\n", 
	     p1->string, p1->val1, p1->val2 );
    p1 = p1->next;
  }
  
  return( 0 );

}/* of fprintstrdoublonglist */

/******************************************************************************
  fprintstrdoublist
return value: 0 (OK)  
******************************************************************************/
int fprintstrdoublist( fp, p )
     FILE *fp;
     struct strdoublist *p;
{
  struct strdoublist *p1;
...
}/* of fprintstrdoublist */

/******************************************************************************
  fprintstrlonglist
return value: 0 (OK)  
******************************************************************************/
int fprintstrlonglist( fp, p )
     FILE *fp;
     struct strlonglist *p;
{
  struct strlonglist *p1;
...
/******************************************************************************
  file2strstrcountlist2

same as file2strstrcountlist except that
after taking the first string,
all of the remaining line is taken as the second string.
e.g.
<string> <prob>...<prob>
=> "string" is the first string, and "<prob>...<prob>" is the 2nd string.

it increments a counter for each new relevant item
and stores it as the EC ID. (but ECs are not used here.)

read two strings from file (as described above) and addtostrstrlonglist

return value: size of strstrlonglist (OK)
             -1 (ERROR)
******************************************************************************/
long file2strstrcountlist2( f, list )
     FILE *f;
     struct strstrlonglist **list;
{
  long retv = 0;
  ...
  while( fgets( line, sizeof( line ), f ) ){
    ...
    *pos = '\0';
    ...
    /*    if( ( p = strtok( NULL, sep ) ) == NULL ){
	  take the rest of the string, not just the 2nd field */
    if( ( p = strtok( NULL, "\n" ) ) == NULL ){
...
add <str1>,<double>,<double>,<double> to end of list

return value: <lastrecord>->next
              NULL (error)
******************************************************************************/
struct strdoub3list **addtostrdoub3list( p, s1, val1, val2, val3 )
     struct strdoub3list **p;
     char *s1;
     double val1;
     double val2;
     double val3;
{
  struct strdoub3list *q;

  if( s1 == NULL )
    return( NULL );

  if( ( q = ( struct strdoub3list * )
	malloc( sizeof( struct strdoub3list ) ) ) == NULL ){
